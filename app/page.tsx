    ...
    <main>
      ...
      <section id="top" className="hero section-pad">
        ...
        <a className="hero-art real-art board-1 crop-c0-r0" href="/artwork/respiro-di-luce" aria-label="Open Respiro di luce by Iliano Babenchini">
          <div className="hero-caption"><span>Respiro di luce · 2023</span><span>80 × 100 cm</span></div>
        </a>
      </section>
        ...
        <div className="art-grid">
          {featured.map((artwork, index) => (
            <a className={`art-card art-card-${index + 1}`} href={`/artwork/${artwork.slug}`} key={artwork.slug}>
              <div className={`artwork-surface real-art board-${artwork.board} ${artwork.crop}`}><span className="view-label">OPEN</span><span className="status-pill">{artwork.status}</span></div>
      ...
      <section id="full-collection" className="archive section-pad">
        ...
        <div className="archive-grid">
          {artworks.map((artwork) => (
            <a className="archive-card" href={`/artwork/${artwork.slug}`} key={artwork.slug}>
              <div className={`archive-image real-art board-${artwork.board} ${artwork.crop}`}><span className="status-pill">{artwork.status}</span></div>
        ...
        <p className="source-note">Collection previews use the supplied studio presentation boards. Individual high-resolution artwork files can replace these previews without changing catalogue metadata.</p>
      </section>

      <section id="artist" className="artist section-pad">
        <div className="portrait-placeholder" aria-hidden="true"><img src="/brand/iliano-babenchini-mark.svg" alt="" /></div>
        <div className="artist-copy"><p className="eyebrow">THE ARTIST</p><h2>Silence, memory and the architecture of inner space.</h2><p>Based in Milan, Iliano Babenchini explores the relationship between human presence, material memory and contemporary interiors. His paintings combine restrained colour, tactile surfaces and a sense of pause.</p><blockquote>“A painting should not fill an empty wall. It should change the atmosphere of the room.”</blockquote><a className="text-link" href="#contact">Discover the studio →</a></div>
      </section>

      <section className="authenticity section-pad">
        <div className="authenticity-copy"><p className="eyebrow">AUTHENTICITY</p><h2>Every original leaves the studio with its identity documented.</h2><p>Each work is accompanied by a signed certificate recording title, year, medium, dimensions and a unique archive number.</p><a className="text-link" href="/brand/certificate-template.svg">View certificate →</a></div>
        <div className="certificate-preview"><img src="/brand/certificate-template.svg" alt="Certificate of authenticity template" /></div>
      </section>
      ...
      <footer className="footer section-pad"><div className="brand-lockup footer-brand"><img className="brand-mark" src="/brand/iliano-babenchini-mark.svg" alt="" /><span className="brand"><span>ILIANO</span><span>BABENCHINI</span></span></div><p>Contemporary Art · Milano</p><p className="footer-note">Private Museum · v0.8.0</p></footer>
    </main>
  );
}
