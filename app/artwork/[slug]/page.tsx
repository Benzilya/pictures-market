  ...
  return (
    <main>
      ...
      <section className="artwork-detail section-pad">
        <div className={`artwork-detail-image real-art board-${artwork.board} ${artwork.crop}`}><span className="status-pill">{artwork.status}</span></div>
        <div className="artwork-detail-copy">
          <p className="eyebrow">{artwork.series} · {artwork.year}</p>
          <h1>{artwork.title}</h1><p className="artwork-price">{artwork.price}</p>
          <dl className="artwork-specs">
            <div><dt>Medium</dt><dd>{artwork.medium}</dd></div><div><dt>Dimensions</dt><dd>{artwork.dimensions}</dd></div><div><dt>Original</dt><dd>Unique work</dd></div><div><dt>Certificate</dt><dd>Included</dd></div><div><dt>Location</dt><dd>Milan, Italy</dd></div>
          </dl>
      ...
      {related.length > 0 && <section className="related section-pad"><div className="section-heading simple-heading"><p className="eyebrow">RELATED WORKS</p><h2>From the same series.</h2></div><div className="related-grid">{related.map((item) => <a href={`/artwork/${item.slug}`} className="collection-tile" key={item.slug}><div className={`collection-tile-image real-art board-${item.board} ${item.crop}`} /><div className="collection-tile-meta"><div><h2>{item.title}</h2><p>{item.year} · {item.dimensions}</p></div><strong>{item.price}</strong></div></a>)}</div></section>}
    </main>
  );
}
